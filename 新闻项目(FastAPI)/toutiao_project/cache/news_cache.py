...
async def get_news_detail(news_id:int) -> Optional[Dict[str,Any]]:
    key = f"{NEWS_DETAIL_PREFIX}{news_id}"
    return await get_json_cache(key)


#缓存相关新闻列表
async def set_related_news(
        news_id:int,
        category_id:int,
        news_detail:Dict[str,Any],
        expire:int=1800
...
async def get_related_news(news_id:int,category_id:int) -> Optional[Dict[str,Any]]:
    key = f"{NEWS_DETAIL_PREFIX}{news_id}:{category_id}"
    return await get_json_cache(key)